import { handleUnaryCall, status as grpcStatus } from '@grpc/grpc-js';

import { VerifyReq, VerifyResp } from '../stub/token_auth_pb';

import { TokenAuthService, ITokenAuthServer } from '../stub/token_auth_grpc_pb';

import * as tokenServ from '../../service/token';

class TokenAuthServer implements ITokenAuthServer {
  [name: string]: import('@grpc/grpc-js').UntypedHandleCall;
  verifyAccessToken: handleUnaryCall<VerifyReq, VerifyResp> = async (call, callback) => {
    console.log('gRPC TokenAuthServer verifyAccessToken invoked');
    ...
    console.log('gRPC TokenAuthServer verifyAccessToken responded');
